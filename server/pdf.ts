...
export async function generatePDF(data: CompleteCV): Promise<Buffer> {
  ...
  // Only using standard layout for all templates now
  ...
  for (const section of sectionOrder) {
    ...
    switch (section.id) {
      ...
      case 'keyCompetencies':
        ...
        // Add consistent spacing after the entire key competencies section
      ...
      case 'additional':
        ...
        if (data.additional && data.additional.skills && data.additional.skills.length > 0) {
          ...
          yPos += (skillsLines.length * lineHeight) + 2; // Reduced spacing between subsections
        }
