...
const PDFPreview: React.FC<PDFPreviewProps> = ({ data, onClose }) => {
  ...
  // Function to download PDF using server-side Puppeteer rendering
  const handleServerPDF = useCallback(async () => {
    try {
      setIsDownloading(true);
      
      // Send the CV data to the server-side API
      ...
      a.href = url;
      a.download = `${data.personal?.firstName || 'CV'}_${data.personal?.lastName || 'Document'}_CV.pdf`;
  ...
  // Function to print PDF using browser's print functionality
