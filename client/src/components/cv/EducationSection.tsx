...
const EducationSection = ({ form }: EducationSectionProps) => {
  ...
  const handleAchievementsChange = (e: React.ChangeEvent<HTMLTextAreaElement>, index: number) => {
    ...
    const prevText = form.getValues(`education.${index}.achievements`);
    ...
    if (text.endsWith('\n') || (text.split('\n').length > prevText.split('\n').length)) {
      ...
      setTimeout(() => {
        ...
        const textarea = document.querySelector(`textarea[name="education.${index}.achievements"]`) as HTMLTextAreaElement;
        if (textarea) {
    ...
    if (currentLineIndex >= 0 && 
      ...
      form.setValue(`education.${index}.achievements`, newText);
