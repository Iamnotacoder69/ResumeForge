  ...
  try {
    ...
    emailIcons.forEach(icon => {
      ...
      if (svg) {
        ...
        if (parent) {
          ...
          const emailSvgHtml = `
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#03d27c" stroke-width="2" style="margin-right: 4px;">
    ...
    linkedinIcons.forEach(icon => {
      ...
      if (svg) {
        const parent = svg.parentElement;
        if (parent) {
          // Create an inline SVG for LinkedIn
          const linkedinSvgHtml = `
            <svg width="16" height="16" viewBox="0 0 24 24" fill="#03d27c" style="margin-right: 4px;">
